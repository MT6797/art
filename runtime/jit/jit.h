...
class Runtime;
bool MTK_UpdateEntrypoints();
void MTK_ZygoteHooks_nativePreFork(Runtime* runtime);
void MTK_EnableDebugFeatures1(Runtime* const runtime);
#endif
...
class MTKJitOptions;
MTKJit* GenMTKJit();
MTKJitOptions* GenMTKJitOptions();
void MTKCreateFromRuntimeArguments1(JitOptions* jit_options);
void MTKJITCreate1(JitOptions* jit_options, Jit* jit);
void MTK_CreateInstrumentationCache(Jit* jit,
                                    jit::JitInstrumentationCache*
                                    instrumentation_cache);
#endif
class Jit {
 public:
  static constexpr bool kStressMode = kIsDebugBuild;
  static constexpr size_t kDefaultCompileThreshold = kStressMode ? 4 : 1000;
...
class JitOptions {
 ...
 public:
  MTKJitOptions* mtk_jit_options_;
 ...
 private:
  ...
  JitOptions() : use_jit_(false), code_cache_capacity_(0), compile_threshold_(0),
      #ifdef MTK_ARTSLIM_ENABLE
      mtk_jit_options_(GenMTKJitOptions()),
      #endif
