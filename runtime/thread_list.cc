...
#ifdef MTK_EXTEND_CHECKPOINT_DUMP_WAIT_TIMEOUT
static constexpr uint32_t kDumpWaitTimeout = 20000;
#else
...
void ThreadList::SuspendAllForDebugger() {
  ...
  if (!Locks::mutator_lock_->ExclusiveLockWithTimeout(self, 300 * 1000, 0)) {
    UnsafeLogFatalForThreadSuspendAllTimeout();
  } else {
    Locks::mutator_lock_->ExclusiveUnlock(self);
  }
