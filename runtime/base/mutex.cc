...
void BaseMutex::DumpContention(std::ostream& os) const {
  if (kLogLockContentions) {
    ...
    if (contention_count == 0) {
      LOG(INFO) << "never contended";
      os << "never contended";
    } else {
      LOG(INFO) << "contended " << contention_count
      ...
      if (max_tid != 0) {
        LOG(INFO) << " sample shows most blocked tid=" << max_tid;
      }
      max_tid = 0;
      max_tid_count = 0;
      ...
      if (max_tid != 0) {
        LOG(INFO) << " sample shows tid=" << max_tid << " owning during this time";
      }
    }
  }
}
...
void Mutex::Dump(std::ostream& os) const {
  LOG(INFO) << (recursive_ ? "recursive " : "non-recursive ")
      << name_
