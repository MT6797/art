 ...
 public:
  BasicBlock(BasicBlockId block_id, BBType type, ArenaAllocator* allocator)
      : id(block_id),
        ...
        p_dominators(nullptr),
        p_i_dominated(nullptr),
        p_dom_frontier(nullptr),
        rev_predecessors(allocator->Adapter(kArenaAllocBBPredecessors)) {
        #else
        successor_blocks(allocator->Adapter(kArenaAllocSuccessor)) {
        #endif
  }
  ...
  ArenaVector<BasicBlockId> rev_predecessors;
#endif
...
#ifdef MTK_ART_COMMON
  void ErasePredecessor(BasicBlockId old_pred, bool update);
  void UpdatePredecessor(BasicBlockId old_pred, BasicBlockId new_pred, bool update);
  void EraseReversePredecessor(BasicBlockId old_pred);
  bool IsPredecessor(BasicBlockId pred);
  void EraseArenaVectorElement(BasicBlockId id, ArenaVector<BasicBlockId> *array);
#endif
...
class MIRGraph {
 public:
  ...
  #ifdef MTK_ART_COMMON
  ArenaVector<BasicBlock*>& GetBlockList() {
  #else
...
#ifdef MTK_ART_COMMON
  ArenaVector<BasicBlockId>& GetReverseDfsOrder() {
    return rev_dfs_order_;
  }

  ArenaVector<BasicBlockId>& GetReverseDfsPostOrder() {
    return rev_dfs_post_order_;
  }

  ArenaVector<BasicBlockId>& GetReverseDomPostOrder() {
 ...
 protected:
  ...
  ArenaVector<BasicBlockId> rev_dfs_order_;
  ArenaVector<BasicBlockId> rev_dfs_post_order_;
  ArenaVector<BasicBlockId> rev_dom_post_order_traversal_;
  int* p_i_dom_list_;
#endif
