...
__attribute__((weak))
bool Optimizing_SkipCompilation(CompilerDriver* compiler_driver ATTRIBUTE_UNUSED, std::string* skip_message ATTRIBUTE_UNUSED,
                        uint32_t method_idx ATTRIBUTE_UNUSED, const DexFile& dex_file ATTRIBUTE_UNUSED, const DexCompilationUnit *dex_compilation_unit ATTRIBUTE_UNUSED) {
  return false;
}
#endif
...
CompiledMethod* OptimizingCompiler::TryCompile(const DexFile::CodeItem* code_item,
  ...
  if (run_optimizations_ && can_optimize && can_allocate_registers) {
...

    {
      ...
      if (Optimizing_SkipCompilation(compiler_driver, &skip_message, method_idx, dex_file, &dex_compilation_unit)) {
        // LOG(INFO) << ": Skipping method by filter: " << PrettyMethod(method_idx, dex_file) << "  Reason = " << skip_message;
  #ifdef MTK_ARTSLIM_ENABLE
        /* If Quick compiler is phased out, we can remove this */
        compiler_driver->skip_by_optimizing_filter = true;
  #endif
        return nullptr;
      }
#endif
...
#ifdef MTK_ARTSLIM_ENABLE
  /* If Quick compiler is phased out, we can remove this */
  compiler_driver->skip_by_optimizing_filter = false;
#endif
...
#ifdef MTK_ARTSLIM_ENABLE
  /* If Quick compiler is phased out, we can remove this */
  if (compiler_driver->skip_by_optimizing_filter == true) {
    return nullptr;
  }
#endif
