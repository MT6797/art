...
class HBasicBlock : public ArenaObject<kArenaAllocMisc> {
 public:
  explicit HBasicBlock(HGraph* graph, uint32_t dex_pc = kNoDexPc)
      : graph_(graph),
        ...
        post_dominated_blocks_(graph->GetArena(), kDefaultNumberOfDominatedBlocks),
        post_dominance_frontier_blocks_(graph->GetArena(), kDefaultNumberOfDominatedBlocks),
#endif
        block_id_(-1),
        dex_pc_(dex_pc),
...
#ifdef MTK_ART_COMMON
  const GrowableArray<HBasicBlock*>& GetPostDominatedBlocks() const {
    return post_dominated_blocks_;
  }

  const GrowableArray<HBasicBlock*>& GetPostDominanceFrontierBlocks() const {
  ...
  void AddPostDominatedBlock(HBasicBlock* block) { post_dominated_blocks_.Add(block); }
  void AddPostDominanceFrontierBlock(HBasicBlock* block) { post_dominance_frontier_blocks_.Add(block); }
#endif
 ...
 private:
  ...
  GrowableArray<HBasicBlock*> post_dominated_blocks_;
  GrowableArray<HBasicBlock*> post_dominance_frontier_blocks_;
#endif
  int block_id_;
...
#define FOR_EACH_INSTRUCTION(M)                                         \
  ...
  FOR_EACH_MTK_INSTRUCTION(M)                                           \
  M(Constant, Instruction)                                              \
  M(UnaryOperation, Instruction)                                        \
  M(BinaryOperation, Instruction)                                       \
  FOR_MTK_TERNARY_INSTRUCTION(M)                                        \
...
class HTernaryOperation : public HExpression<3> {
 public:
  ...
  virtual int32_t Evaluate(int32_t x, int32_t y, int32_t z) const = 0;
  virtual int64_t Evaluate(int64_t x, int64_t y, int64_t z) const = 0;
...
class HMla : public HTernaryOperation {
 public:
  ...
  int32_t Evaluate(int32_t x, int32_t y, int32_t z) const OVERRIDE { return x * y + z; }
  int64_t Evaluate(int64_t x, int64_t y, int64_t z) const OVERRIDE { return x * y + z; }
...
#ifdef MTK_ART_COMMON
  ClinitCheckRequirement GetClinitCheckRQ() { return clinit_check_requirement_; }
...
class HPow : public HBinaryOperation {
 public:
  HPow(Primitive::Type result_type, HInstruction* left, HInstruction* right, uint32_t dex_pc)
      : HBinaryOperation(result_type, left, right), dex_pc_(dex_pc) {}
...
class HZeroBranch : public HExpression<1> {
 public:
  HZeroBranch(HInstruction* value, uint32_t dex_pc)
      :HExpression(value->GetType(), SideEffects::None()), dex_pc_(dex_pc) {
    SetRawInputAt(0, value);
  }
