...
#ifdef MTK_ART_COMMON
  const ArrayIndexList& GetArrayIndexList() const;
  bool Skip() const;
  bool FindArrayIndex(HInstruction* index);
  void AddArrayIndex(HInstruction* index, HArrayLength* array_length);
#endif
...
#ifdef MTK_ART_COMMON
        } else if (index->IsAdd() || index->IsSub()) {
          AddArrayIndex(index, array_length);
          // Dont't pollute the original logic
          continue;
#endif
        } else {
...
#ifdef MTK_ART_COMMON
  ValueRange* NarrowWithDeoptimization(ArrayRangeList &range_list) {
#else
...
#ifdef MTK_ART_COMMON
    AddArrayRanges(range_list, finder.GetArrayIndexList());
#endif
...
#ifdef MTK_ART_COMMON
  bool CanAddDeoptimizationVariable(const ArrayIndexList &arr_idx_list) const;
  void AddDeoptimizationVariables(HInstruction* value,
                                  const ArrayIndexList &arr_idx_list) const;
  void AddDeoptimizationVariable(HInstruction* value,
                                 HArrayLength* array_length,
                                 HInstruction* offset) const;
  void AddArrayRanges(ArrayRangeList &range_list, const ArrayIndexList &index_list);
#endif
...
#ifdef MTK_ART_COMMON
    if (!finder.Skip()) {
      if (CanAddDeoptimizationVariable(finder.GetArrayIndexList())) {
        AddDeoptimizationVariables(end_, finder.GetArrayIndexList());
        return true;
      }
      return false;
    }
...
#ifdef MTK_ART_COMMON
                NarrowWithDeoptimization(arr_rgn_list);
#else
...
#ifdef MTK_ART_COMMON
              PropagateArrayRanges(instruction->IfFalseSuccessor());
#endif
            }
          }
        }
